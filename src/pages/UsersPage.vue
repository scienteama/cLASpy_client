<template>
  <q-page padding>
    ...
    <div class="q-pa-sm">
      <!-- Table desktop -->
      <q-table v-if="!$q.screen.lt.md" flat bordered :rows="users" :columns="columns"
        :rows-per-page-options="[0, 5, 10, 20]" row-key="id" class="responsive-table">
            ...
            <q-td v-for="col in props.cols" :key="col.name" :props="props" class="q-px-sm">
              <template v-if="col.name === 'actions'">
                <div class="row no-wrap items-center q-gutter-sm">
                  <q-btn v-if="userStore.isPrivileged" flat icon="edit" color="primary"
                    @click="openEditDialog(props.row)" dense size="sm" />
                  <q-btn v-if="userStore.isAdmin" flat icon="delete" color="negative" @click="confirmDelete(props.row)"
                    :disabled="props.row.id === userStore.currentUser?.id" dense size="sm">
                    <q-tooltip v-if="props.row.id === userStore.currentUser?.id" transition-show="flip-right"
        ...
        <q-card style="width: 95vw; max-width: 500px;">
          <q-form @submit.prevent="submitUserForm" @reset.prevent="resetUserForm">
            ...
            <q-card-section class="q-gutter-md">
              ...
              <q-input v-model="formUser.email" label="Email" type="email" dense autocomplete="off" lazy-rules
                :rules="[formUserRules.required('Email'), formUserRules.email]" />

              <!-- Bouton dՎdition du mot de passe -->
              <div class="q-mb-md">
                <q-btn v-if="editingUser" flat dense :label="!passwordChanged ? 'Modifier le mot de passe' : 'Annuler'"
                  color="primary" @click="passwordChanged = !passwordChanged" />
              </div>

              <q-input v-if="!editingUser || passwordChanged" v-model="newPassword" filled
                :type="isPwd ? 'password' : 'text'" autocomplete="new-password" label="Mot de passe" dense
                placeholder="**************" :rules="getPasswordRules()">
